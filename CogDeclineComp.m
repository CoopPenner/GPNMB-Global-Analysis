...
%% reading in data
dataTable=readtable('/Volumes/PC60/InqueryDatasets/FinalizedSets/ADC_MMSE_TotalPatients.xlsx');

%dataTable=readtable('/Volumes/PC60/InqueryDatasets/FinalizedSets/ADC_MMSE_ActivePatient.xlsx');
...
modelData = table( ...
    ...
    categorical(apoeHaplo(filter2use)), ...
    scoreToTest(filter2use), ...
    startScoreMat(filter2use), ...
    'VariableNames', {'ptID', 'timePassed', 'Sex', 'ageAtTest', 'SNP', 'APOE', 'Scores', 'startScore'} ...
);
...
glmeAlt = fitglme(modelData, ...
    'Scores ~ 1 + Sex + startScore + ageAtTest + APOE + (1|ptID)', ...
    'Distribution','normal','Link','identity');
